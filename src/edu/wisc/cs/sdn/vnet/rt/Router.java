...
/**
 * @author Yi-Shiun Chang and Shuo Han
 */
public class Router extends Device {
	...
	/**
	 * Destination Ethernet address should be the broadcast MAC address
	 * (FF:FF:FF:FF:FF:FF)
	 */
	private final static byte[] RIP_BROADCAST_MAC = new byte[] { (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff,
			(byte) 0xff, (byte) 0xff };
	...
	/**
	 * When a static route table is not provided for current router, this method
	 * build RIP to build a route table.
	 */
	public void startRIP() {
		// when the current router starts, add entries to the route table for the
		// subnets that are directly reachable via the router’s interfaces
		// router should send a RIP request out all of the current router’s interfaces
		// when RIP is initialized
		for (Iface iface : this.interfaces.values()) {
			...
			// the destination IP address should be the IP address reserved for RIP
			// (224.0.0.9)
			// the destination Ethernet address should be the broadcast MAC address
			// (FF:FF:FF:FF:FF:FF).
			// generateRIPv2Packet(Iface iface, int destIP, byte[] destMAC, boolean
			// isRequest)
		...
		// your router should send an unsolicited RIP response out all of the router’s
		// interfaces every 10 seconds
		// your router should time out route table entries for which an update has not
		// been received for more than 30 seconds.
		// You should never remove route entries for the subnets that are directly
		// reachable via the router’s interfaces .
		...
		Runnable cleanup = new Runnable() {
			public void run() {
				try {
					...
					// when multiple threads want to access ripTable, synchronized makes sure only
					// one thread thread can do it at a moment
					synchronized (ripTable) {
						// when multiple threads want to access routeTable, synchronized makes sure only
						// one thread thread can do it at a moment
	...
	/**
	 * The RIPv2 and RIPv2Entry classes in the net.floodlightcontroller.packet
	 * package define the format for RIPv2 packets.
	 * 
	 * When sending a RIP response for a specific RIP request, the destination IP
	 * address and destination Ethernet address should be the IP address and MAC
	 * address of the router interface that sent the request.
	 * 
	 */
	private Ethernet generateRIPv2Packet(Iface iface, int destIP, byte[] destMAC, boolean isRequest) {
		...
		// whose source and destination ports are 520 (defined as a constant RIP_PORT in
		// the UDP class)
		UDP udp = new UDP();
	...
	public void handlePacket(Ethernet etherPacket, Iface inIface) {
		...
		case Ethernet.TYPE_IPv4:
			...
			break;
		// to accept ARP packets (i.e., the EtherType field in the Ethernet header
		// equals Ethernet.TYPE_ARP)
	...
	private void handleArpPacket(Ethernet etherPacket, Iface inIface) {
		...
		// if an ARP packet is an ARP request (i.e., the opcode field in the ARP header
		// equals ARP.OP_REQUEST),
		...
		if (arpPacket.getOpCode() == ARP.OP_REQUEST) {
			...
			// your router must only respond to ARP requests whose target IP protocol
			// address equals the IP address of the interface
			// on which the ARP request was received.
			for (Iface iface : this.interfaces.values()) {
				if (targetIp == iface.getIpAddress()) {
					Ethernet ether = ARPReply(etherPacket, arpPacket, inIface);
					// Once the ARP reply is fully constructed, you should send it on the same
	...
	private void handleIpPacket(Ethernet etherPacket, Iface inIface) {
		...
		// if the frame contains an IPv4 packet, verify the checksum and time-to-live
		// (TTL) of the
		// IPv4 packet. Use the getPayload() method of the Ethernet class to get the
		// IPv4 header.
		...
		// the length of the IP header can be determined from the header length field in
		// the IP header,
		// the checksum field in the IP header should be zeroed before calculating the
		// use code from the serialize() method in the IPv4 class to compute the
		// IP checksum.
		// checksum.
		...
		// check TTL
		// Time-to-live (TTL) or hop limit is a mechanism that limits the lifespan or
		// lifetime of data
		// in a computer or network. TTL may be implemented as a counter or timestamp
		// attached to or
		// embedded in the data. Once the prescribed event count or timespan has
		// elapsed, data is
		// discarded or revalidated.
		// in computer networking, TTL prevents a data packet from circulating
		// indefinitely.
		...
		// ICMP message should be sent if your router receives a TCP or UDP packet
		// destined for one of its interfaces.
		for (Iface iface : this.interfaces.values()) {
			if (ipPacket.getDestinationAddress() == iface.getIpAddress()) {
				// if a TCP or UDP header comes after the IP header (i.e., the protocol field in
				// the IP header equals IPv4.PROTOCOL_UDP or IPv4.PROTOCOL_TCP)
				// then you should construct and send destination port unreachable message.
				// you should drop the original packet after sending the destination port
				// unreachable message.
				if (ipPacket.getProtocol() == IPv4.PROTOCOL_TCP || ipPacket.getProtocol() == IPv4.PROTOCOL_UDP) {
					...
					return;
					// if an ICMP header comes after the IP header (i.e., the protocol field in the
					// IP header equals IPv4.PROTOCOL_ICMP),
					// then you should check if the ICMP message is an echo request (i.e., the type
					// field in the ICMP header equals 8).
					// if the ICMP message is an echo request (used by ping),
					// then you should construct and send an echo reply message, otherwise, you
	...
	private void handleRipPacket(Ethernet etherPacket, Iface inIface) {
		...
		// should be the IP address and MAC address of the router interface that sent
		// the request
		if (rip.getCommand() == RIPv2.COMMAND_REQUEST) {
			sendPacket(generateRIPv2Packet(inIface, ip.getSourceAddress(), etherPacket.getSourceMACAddress(), false),
					inIface);
			return;
		}
		...
		for (RIPv2Entry entry : rip.getEntries()) {
			...
			if (ripTable.containsKey(subnetNumber)) {
				if (newCost < ripTable.get(subnetNumber).cost) {
					...
					// update routeTable
					if (!routeTable.update(entry.getAddress(), entry.getSubnetMask(), entry.getNextHopAddress(),
							inIface)) {
						routeTable.insert(entry.getAddress(), entry.getNextHopAddress(), entry.getSubnetMask(),
								inIface);
					}
				} else {
				...
				// update routeTable
				if (!routeTable.update(entry.getAddress(), entry.getSubnetMask(), entry.getNextHopAddress(), inIface)) {
	...
	private void forwardIpPacket(Ethernet etherPacket, Iface inIface) {
		...
		// if no entry matched, generate an ICMP destination net unreachable message
		// prior to dropping the packet
		if (null == bestMatch) {
		...
		// all packet headers should be complete, except the destination MAC address in
		// the Ethernet header, before enqueueing the packet.
		// your router should call the lookup() function of the ARPCache class when
		// trying to forward packets,
		// and if no entry is found, generate a destination host unreachable ICMP
		// message and drop the packet.
		ArpEntry arpEntry = this.arpCache.lookup(nextHop);
		// your router enqueues the packet and generates an ARP request if no matching
		// entry is found in the ARP cache
		if (arpEntry == null) {
			// if the router wants to forward other IP packets to the same IP that your
			// router is already trying to resolve,
			// you should simply add those packets to the queue for the corresponding IP
			// address.
			if (PacWaitMAC.containsKey(nextHop)) {
				PacWaitMAC.get(nextHop).add(etherPacket);
				// router maintains a separate queue of packets for each IP address for which we
				// are waiting for the corresponding MAC address
			} else {
	...
	/**
	 * when the router generates ICMP messages, the packet must contains an Ethernet
	 * header, IP header, ICMP header, and ICMP payload. You can construct these
	 * headers, by creating Ethernet, IPv4, ICMP, and Data objects by using the
	 * classes in the net.floodlightcontroller.packet package.
	 * 
	 * To link the headers together, you should call the setPayload(..) method
	 * defined in the BasePacket class, which is the superclass for all of the
	 * header classes. Below is a snippet of code to get you started:
	 * 
	 * @param ipPacket
	 * @param inIface
	 * @param outIface
	 ...
	 * 
	 * ICMP message should be sent when your router receives an ICMP echo request
	 * destined for one of its interfaces. you should not send an ICMP echo reply if
	 * you receive an echo request whose destination IP address does not match any
	 * of the IP addresses assigned to the router’s interfaces; these packets should
	 * be forwarded as they were before, since they are destined for hosts (or other
	 * routers).
	 * 
	 * the ICMP echo reply message should be constructed similar to the time
	 * exceeded message. however, the source IP in the IP header should be set to
	 * the destination IP from the IP header in the echo request. additionally, the
	 * type field in the ICMP header should be set to 0. lastly, the payload that
	 * follows the ICMP header in the echo reply must contain the entire payload
	...
	private Ethernet ARPReply(Ethernet etherPacket, ARP arpPacket, Iface inIface) {
		...
		// Source MAC — set to the MAC address of the interface on which the original
		// packet arrived
		...
		// Sender hardware address — set to the MAC address of the interface on which
		// the original packet arrived
		// Sender protocol address — set to the IP address of the interface on which the
		// original packet arrived
		// Target hardware address — set to the sender hardware address from the
		// original packet
		// Target protocol address — set to the sender protocol address from the
		// original packet
		ARP arp = new ARP();
		...
		// link the ethernet header and arp header together using the setPayload()
	...
	private Ethernet ARPRequest(Ethernet etherPacket, Iface inIface, int nexthopIP) {
		// an ARP request should be construct similar to an ARP reply, except for the
		// following specifics
		...
		// Target protocol address — set to the IP address whose associated MAC address
		// we want
		ARP arp = new ARP();
		arp.setHardwareType(ARP.HW_TYPE_ETHERNET);
		arp.setProtocolType(ARP.PROTO_TYPE_IP);
	 ...
	 * @param nextHop
	 */
	private void sendARPrequests(final Ethernet etherPacket, final Iface inIface, final Iface outIface,
			final int nextHop) {
		...
		final Runnable ARPrequest = new Runnable() {
			...
			public void run() {
				...
				// your router either receives a corresponding ARP reply or you have sent the
				// same ARP request 3 times.
				if (arpEntry == null) {
					// if you have sent an ARP request 3 times and no corresponding ARP reply has
					// been received 1 second
					// after sending the third request, then you should drop any packets that were
					// queued waiting for a reply,
					// however, before dropping each packet, you should generate a destination host
					// unreachable message.
					if (sendTimes == 3) {
						...
						// if a corresponding ARP reply does not arrive within 1 second of issuing the
						// ARP request,
						// then you should send another ARP request that’s exactly the same as the
